...
再想一步：你发的中文会翻成 `wp`、`ty`、`omw`、`inc mid` 这类很短的英文，
而回显名单里的内容是整局留着的 —— 那么别人 10 分钟后说的同一句 `wp`，
就会被当成"你的回显"而永远不翻译。所以 v1.0.7 起加了一道 **15 秒时间窗**：
只有 15 秒内自己发过的内容才用来认领回显。回显本身是紧接着发送到达的（往返通常不到 1 秒），
所以该认的一条都不会漏。
